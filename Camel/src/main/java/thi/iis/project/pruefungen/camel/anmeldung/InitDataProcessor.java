...
/**
 * Processor that gets object of type InputData, extracts necessary information
 * and sets them as parameter of outgoing message
 * 
...
public class InitDataProcessor implements Processor {
    ...
    public void process(Exchange exchange) throws Exception {
        ...
        for (Deadline d : deadlineList) {
            newData.getProcessVariables()
                    .add(new ProcessVariable(d.getDeadlineName(), new ValueType(d.getDate(), "Date")));
        }
        // set examlist
        ExamList examList = new ExamList();
        examList.setExams(in.getExamList());
        newData.getProcessVariables()
                .add(new ProcessVariable("examList", new ValueType(examList.toJson().toString(), "String")));
        // set student List
        StudentList studentList = new StudentList();
        studentList.setStudents(in.getStudentList());
        newData.getProcessVariables()
                .add(new ProcessVariable("studentList", new ValueType(studentList.toJson().toString(), "String")));
        // set number of students
        newData.getProcessVariables()
                .add(new ProcessVariable("numberOfStudents", new ValueType(in.getNumberOfStudents(), "Integer")));
        // set first exam date
        newData.getProcessVariables()
                .add(new ProcessVariable("firstExamDate", new ValueType(in.getFirstExamDate(), "Date")));
